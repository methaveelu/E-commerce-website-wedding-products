...
router.post("/create-shop", upload.single("file"), async (req, res, next) => {
  try {
    ...
    const activationUrl = `https://e-commerce-client-0xts.onrender.com/seller/activation/${activationToken}`;
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!isPasswordValid) {
        return next(
          new ErrorHandler("Please key in the correct password!", 400)
        );
      }
...
router.delete(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!seller) {
        return next(
          new ErrorHandler("Seller with this ID not found!", 400)
        );
      }
